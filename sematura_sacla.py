...
class DiffuseImage(DiffuseExperiment):
    ...
    def set_general_variables(self):
        ...
        for panel in self.detector:
            ...
            self.lab_coordinates.append(panel.get_lab_coord(mms))
            ...
            #self.pixel_size = panel.get_pixel_size()[0]  # XXX
    ...
    def remove_bragg_peaks(self, radial=None, write_lunus=None):
        ...
        self.lunus_data = []
        self.lunus_data_scitbx = []

        data = self.raw_data
        if len(self.detector) == 0:
            data = data,

        for panel_id in xrange(len(self.detector)):
            image = LunusDIFFIMAGE()
            image.set_image(data[panel_id])
            ...
            beam_fast, beam_slow = self.detector[panel_id].get_beam_centre(self.s0)
            distance = self.detector[panel_id].get_distance()
            pixel_size = self.detector[panel_id].get_pixel_size()[0] # assumes pixels are square

            image.LunusPolarim(beam_fast, beam_slow, distance,
                               self.polarization_fraction, self.polarization_offset, pixel_size)
            print "Solid-angle normalization of image"
            image.LunusNormim(beam_fast, beam_slow, distance, self.cassette_x, self.cassette_y, pixel_size)
            
            self.lunus_data_scitbx.append(image.get_image());

            self.radial_avg = image.LunusRadialAvgim(beam_fast, beam_slow, pixel_size)

            self.lunus_data.append(self.lunus_data_scitbx[panel_id].as_numpy_array())
            print("shape of lunus data at step 1: {}".format(self.lunus_data[panel_id].shape))
    ...
    def crystal_geometry(self, xtal):
        ...
        self.r_vectors = []
        for panel_id in xrange(len(self.detector)):
            s1_vectors = self.lab_coordinates[panel_id].each_normalize() * (1/self.beam.get_wavelength())
            self.r_vectors.append(s1_vectors - self.beam.get_s0())
    ...
    def panel_to_lattice(self, panel_id = 0):
        ...
        H = np.tensordot(a_inv, self.r_vectors[panel_id], (1,1))
        H = np.transpose(H)
    ...
    def integrate_diffuse(self):
        ...
        for panel_id, panel in enumerate(self.detector):
            ...
            print "there are ",self.detector[panel_id].get_image_size()[0] * \
                               self.detector[panel_id].get_image_size()[1]," pixels in this diffraction image"
            # if len(detector) > 1:
            #     DATA = img.get_raw_data(panel_id)
            # else:
            #     DATA = img.get_raw_data()
            tmp_latit = self.panel_to_lattice(panel_id)
            if latit is None:
                latit = tmp_latit
            else:
                latit += tmp_latit
